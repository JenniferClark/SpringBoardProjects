...
SELECT COUNT(*) AS free_to_members 
FROM Facilities 
WHERE membercost = 0;
...
the member name. */
SELECT DISTINCT f.name as court_name, (m.firstname || ' ' || m.surname) as member 
FROM Bookings b 
...
JOIN Facilities f ON b.facid == f.facid 
JOIN Members m ON b.memid == m.memid 
WHERE DATE(b.starttime) = '2012-09-14' AND (CASE WHEN b.memid = 0 THEN b.slots * f.guestcost ELSE b.slots * f.membercost END) > 30 
ORDER BY cost DESC;
...
WHERE booking_date = '2012-09-14' AND cost > 30
ORDER BY cost DESC;
